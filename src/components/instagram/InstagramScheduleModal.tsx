...
export function InstagramScheduleModal({ isOpen, onClose }: InstagramScheduleModalProps) {
  ...
  // Cargar turnos y reservas de la semana seleccionada
  const loadWeekData = useCallback(async () => {
    ...
    if (!db) {
      ...
      const localBookings = (fallbackBookings || []).filter(
        (b) => b.shiftDate >= startDate && b.shiftDate <= endDate && b.status === "confirmed"
      );
    ...
    try {
      ...
      setBookings((fallbackBookings || []).filter((b) => b.shiftDate >= startDate && b.shiftDate <= endDate));
    } finally {
      setIsLoading(false);
    }
  }, [isOpen, allWeekDays, fallbackShifts, fallbackBookings]);
